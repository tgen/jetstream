...
For python-package plugins it's easy, just define a set of values that need to
be included in __init__. For script plugins it's much harder because we don't
ead them at runtime, they're just executed. We could require a manifest.json be
...
The tools here can be combined for some pretty cool results. Here is a loop
that prints every version of a plugin component available in the archive:

```python
from jetstream import plugins
for r in plugins.list_revisions('plugin.git/script.yaml'):
     p = plugins.get_plugin('plugin.git/script.yaml:{}'.format(r['id']))
     print('plugin.git/script.yaml:{}'.format(['id']))
     print(p.decode())
```
...
def plugin_id(plugin, path, revision):
