...
class ConfigAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None, **kwargs):
        ...
        try:
            ...
            msg = (
                f'No function found for "{var_type}". Available types'
...
def arg_parser():
    ...
    parser.add_argument(
        "-v", "--version", action="version", version=jetstream.__version__
    )
    ...
    subparser = parser.add_subparsers(
        dest="subcommand", help=jetstream.cli.subcommands.__doc__
    )
        ...
        if args.config_file:
            if args.config_file_type:
                config_file = jetstream.utils.load_file(
                    args.config_file, filetype=args.config_file_type
                )
            else:
